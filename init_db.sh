...
if [ -f /docker-entrypoint-initdb.d/init_schema.sql ]; then
    echo "kubota_db にテーブルを作成中..."
    mysql --default-character-set=utf8mb4 -u root -p"${MYSQL_ROOT_PASSWORD}" kubota_db < /docker-entrypoint-initdb.d/init_schema.sql
...
if [ -f /docker-entrypoint-initdb.d/init_schema.sql ]; then
    ...
    sed 's/USE kubota_db;/USE tiera_db;/g' /docker-entrypoint-initdb.d/init_schema.sql | mysql --default-character-set=utf8mb4 -u root -p"${MYSQL_ROOT_PASSWORD}"
