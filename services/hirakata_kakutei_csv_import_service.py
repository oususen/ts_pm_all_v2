...
class HirakataKakuteiCSVImportService:
    """枚方向け確定受注CSVインポートサービス（RCV_NVAN形式）"""

    HISTORY_PREFIX = "[枚方様・確定CSV]"
    ...
    def _group_by_product_and_date(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for _, row in df.iterrows():
            ...
            # 列9: 品名
            product_name = str(row[8]).strip() if len(row) > 8 else ''

            # 列11: 検区
            inspection_category = str(row[10]).strip() if len(row) > 10 else 'N'
            if not inspection_category or inspection_category.lower() == 'nan':
                inspection_category = 'N'

            # 列20: 納入指示数（インデックス19）
            quantity = self._safe_int(row[19]) if len(row) > 19 else 0
            if quantity <= 0:
                continue
            ...
            # 列19: 納入指示日（YYMMDD形式、インデックス18）
            delivery_date_str = str(row[18]).strip() if len(row) > 18 else ''

            delivery_date = self._parse_delivery_date(delivery_date_str)
            if not delivery_date:
                continue

            customer_code = 'HIRAKATA_K'
            customer_name = '枚方様(確定)'
    ...
    def _create_delivery_progress(self, grouped_data: List[Dict],
        ...
        try:
            for item in grouped_data:
                ...
                order_id = f"HIRAKATA-KAKUTEI-{delivery_date.strftime('%Y%m%d')}-{product_code}-{inspection_category}"
                ...
                params = {
                    ...
                    'customer_code': item['customer_code'] or 'HIRAKATA_K',
                    'customer_name': '枚方様(確定)',
                    'order_type': '確定',
                    'order_number': order_numbers_str,
                    'notes': notes_base,
    ...
    def _parse_delivery_date(delivery_str: str) -> Optional[date]:
        """納入指示日（YYMMDD形式想定）をdate化"""
        if not delivery_str or delivery_str.lower() == 'nan':
            return None
        delivery_str = delivery_str.strip()

        # YYMMDD形式（6桁）
        if len(delivery_str) == 6 and delivery_str.isdigit():
            year = 2000 + int(delivery_str[:2])
            month = int(delivery_str[2:4])
            day = int(delivery_str[4:6])
            try:
